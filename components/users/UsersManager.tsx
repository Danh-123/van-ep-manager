...
export default function UsersManager({ currentUserId }: UsersManagerProps) {
  ...
  const totalLabel = useMemo(() => `${filteredCount} người dùng`, [filteredCount]);
  ...
  const validateCreate = () => {
    if (!createEmail.trim()) return 'Email là bắt buộc';
    if (!/^\S+@\S+\.\S+$/.test(createEmail.trim())) return 'Email không hợp lệ';
    if (!createFullName.trim() || createFullName.trim().length < 2) return 'Họ tên phải từ 2 ký tự trở lên';
    if (createPassword.length < 8) return 'Mật khẩu tạm thời phải từ 8 ký tự trở lên';
    return null;
  };
    ...
    <div className="space-y-5">
        ...
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[95vw] max-w-xl -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              <Dialog.Title className="text-lg font-semibold text-slate-900">Tạo người dùng mới</Dialog.Title>
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="mb-1 block text-sm text-slate-700">Họ tên</label>
                <input
              ...
              <div>
                <label className="mb-1 block text-sm text-slate-700">Mật khẩu tạm thời</label>
                <input
                  type="password"
              ...
              <div>
                <label className="mb-1 block text-sm text-slate-700">Vai trò</label>
                <select
                  value={createRole}
      ...
      <Dialog.Root open={editOpen} onOpenChange={setEditOpen}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[95vw] max-w-xl -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              <Dialog.Title className="text-lg font-semibold text-slate-900">Cập nhật người dùng</Dialog.Title>
              <Dialog.Close asChild>
                <button type="button" className="rounded-lg p-1.5 text-slate-500 hover:bg-slate-100">
                  <X className="h-4 w-4" />
                </button>
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  {selectedUser.id === currentUserId && (
                    <p className="mt-1 text-xs text-amber-700">Bạn không thể thay đổi vai trò của chính mình.</p>
                  )}
                </div>
              </div>
            )}
      ...
      <Dialog.Root open={resetOpen} onOpenChange={setResetOpen}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[95vw] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">
            <Dialog.Title className="text-lg font-semibold text-slate-900">Đặt lại mật khẩu</Dialog.Title>
            <p className="mt-2 text-sm text-slate-600">
              Gửi email đặt lại mật khẩu cho <span className="font-medium text-slate-900">{selectedUser?.email ?? '-'}</span>?
            </p>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">
                  Bạn chắc chắn muốn xóa người dùng <span className="font-medium text-slate-900">{selectedUser?.email ?? '-'}</span>?
                </p>
                <p className="mt-1 text-xs text-red-700">
                  Nếu người dùng đã có dữ liệu liên quan, hệ thống sẽ tự động chặn thao tác xóa.
                </p>
              </div>
            </div>
