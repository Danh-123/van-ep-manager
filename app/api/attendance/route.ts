...
export async function GET(request: NextRequest) {
  ...
  const attendanceResult = await supabase
    .from('cham_cong')
    .select('cong_nhan_id, so_luong, ghi_chu')
  ...
  (attendanceResult.data ?? []).forEach((row) => {
    ...
    const status = meta.status ?? (Number(typed.so_luong) > 0 ? 'CoMat' : 'Nghi');
...
export async function POST(request: NextRequest) {
  ...
  for (const row of rows) {
    const present = isPresent(row.status);
    const record = {
      cong_nhan_id: row.congNhanId,
      ngay: date,
      so_luong: present ? 1 : 0,
      don_gia: present ? salaryPerPerson : 0,
  ...
  const recalculateResult = await supabase.rpc('recalculate_luong_ngay', {
    p_ngay: date,
  });
