...
function groupByCustomer(rows: DebtCalculatedRow[]): TruckDebtSummaryRow[] {
  const map = new Map<string, TruckDebtSummaryRow>();

  rows.forEach((row) => {
    const customer = row.khachHang?.trim() || 'Khach le';
...
export async function exportTrucksExcel(rows: DebtCalculatedRow[]) {
  const workbook = new ExcelJS.Workbook();

  const listSheet = workbook.addWorksheet('Danh sách phiếu cân');
  listSheet.columns = [
    { header: 'Ngày', key: 'ngay', width: 14 },
    { header: 'Biển số', key: 'bienSo', width: 16 },
    { header: 'Khách hàng', key: 'khachHang', width: 24 },
    { header: 'Số tấn', key: 'soTan', width: 12 },
  ...
  ['donGia', 'thanhTien', 'congNo', 'thanhToan', 'conLai'].forEach((key) => {
